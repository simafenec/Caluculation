        ...
        public string Calc(string formula)
        {
            ...
            else
            {
                ...
                if (exist_op_times != -1) { //掛け算記号が式の中にあったときの処理
                    int exist_op_flont = Math.Max(Math.Max(Search_op(formula,op[3],start,exist_op_times-1),Search_op(formula, op[4], start, exist_op_times - 1)),Search_op(formula,op[5],start,exist_op_times-1));//前に記号があればその場所を指す
                    ...
                    int[] exist_op_back = new int[] { exist_op_plus_back, exist_op_minus_back, exist_op_div_back };

                    if (exist_op_plus_back ==-1&& exist_op_minus_back==-1&&exist_op_div_back==-1) {
                        //掛け算記号の後ろに計算記号がないとき
                        if (exist_op_flont == -1)
                        {
                            //前にも記号がないとき　
                            ...
                            //前には記号があるとき
                            int x = int.Parse(Splitstring(formula, exist_op_flont + 1, exist_op_times - 1));
                            int y = int.Parse(Splitstring(formula, exist_op_times + 1, end));
                            char c = formula[exist_op_times];
                            ans = Super_Easy_calc(x, y, c);
                            return_value = Splitstring(formula, start, exist_op_flont) + ans;
                            return Calc(return_value);
                        }
                    }
                    else{
                        ...
                        else
                        {
                            ...
                            char c = formula[exist_op_times];
                            ans = Super_Easy_calc(x, y, c);
                            return_value = Splitstring(formula, start, exist_op_flont) + ans+Splitstring(formula,min_op_pos,end);
                            return Calc(return_value);
                        }
                    }
                ...
                if (exist_op_div != -1) {
                    //割り算記号があった時の処理
                    int exist_op_flont = Math.Max(Search_op(formula,op[4],start,exist_op_div-1),Search_op(formula,op[5],start,exist_op_div+1));
                    ...
                    if (exist_op_flont != -1) {
                        ...
                        {
                            //後ろに記号がなかった時
                            int x = int.Parse(Splitstring(formula, exist_op_flont + 1, exist_op_div - 1));
